...
SELECT customer.customer_id, customer.last_name, customer.first_name, CONCAT('$', FORMAT(SUM(payment.amount), 2)) 'total paid'
	FROM customer
...
SELECT store.store_id, CONCAT('$', FORMAT(SUM(payment.amount), 2)) `Store Business`
	FROM store
